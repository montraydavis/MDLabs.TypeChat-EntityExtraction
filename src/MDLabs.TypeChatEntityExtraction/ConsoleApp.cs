...
namespace MDLabs.TypeChatEntityExtraction
{
    ...
    public abstract class ConsoleApp : IInputHandler
    {
        ...
        public virtual async Task RunAsync(string consolePrompt, string? userPrompt = null, string? inputFilePath = null)
        {
        ...
        public virtual async Task RunAsync(string? userPrompt = null, CancellationToken cancelToken = default)
        {
                Console.Write(ConsolePrompt);
                string? input = userPrompt ?? await ReadLineAsync(cancelToken).ConfigureAwait(false);
                input = input.Trim();
